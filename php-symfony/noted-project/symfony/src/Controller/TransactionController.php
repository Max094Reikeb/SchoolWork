...
class TransactionController extends AbstractController
{
    ...
    #[Route('/new/{id}', name: 'app_transaction_new_id', methods: ['GET', 'POST'])]
    public function newForPerson(Request $request, EntityManagerInterface $entityManager, BalanceService $balanceService, Person $person): Response
    {
        $transaction = new Transaction();
        $form = $this->createForm(TransactionType::class, $transaction, [
            'person' => $person,
    ...
    public function delete(Request $request, Transaction $transaction, EntityManagerInterface $entityManager, BalanceService $balanceService): Response
    {
        if ($this->isCsrfTokenValid('delete' . $transaction->getId(), $request->request->get('_token'))) {
            $balanceService->reverseTransaction($transaction);
